...
class KitBot {
  ...
  constructor ({ config, notifier, logger, onEnd, state = null }) {
    this.config = config
  ...
  #onWhisper (username, message) {
    ...
    const [verb, ...args] = message.trim().split(/\s+/)
    ...
    switch (command) {
      ...
      case 'sethome':
        this.#setHome(username, args)
        break

      case 'home':
        this.#reportHome(username)
        break

      case 'clearhome':
        this.#clearHome(username)
        break

      case 'help':
        this.#bot.whisper(username, 'Commands: sethome [x z], home, clearhome, queue, tpa, stop')
        break

      default:
        this.#bot.whisper(username, `Unknown command "${command}". Try "help".`)
    }
  }

  /**
   * `sethome` records where the bot is standing; `sethome <x> <z>` takes it
   * literally. Either way it is persisted, so it survives a restart.
   */
  #setHome (username, args) {
    let home

    if (args.length === 0) {
      if (!this.#bot.entity) {
        this.#bot.whisper(username, "I don't know where I am yet; try again once I've spawned.")
        return
      }
      const { x, z } = this.#bot.entity.position
      home = { x: Math.round(x), z: Math.round(z) }
    } else if (args.length === 2) {
      const x = Number(args[0])
      const z = Number(args[1])
      if (!Number.isFinite(x) || !Number.isFinite(z)) {
        this.#bot.whisper(username, 'Usage: sethome  (where I stand), or sethome <x> <z>')
        return
      }
      home = { x, z }
    } else {
    ...
    this.config.kit.home = home

    const persisted = this.state?.set('kitHome', home) ?? false
    const note = persisted ? '' : ' (could not save it to disk, so it resets on restart)'
    this.#bot.whisper(username, `Home set to x=${home.x} z=${home.z}${note}`)
    this.log.info(`Owner ${username} set home to x=${home.x} z=${home.z}`)
    this.notifier.send(`🏠 **${username}** set the bot's home to \`${home.x}, ${home.z}\`.`)
  }

  #reportHome (username) {
    const home = this.config.kit.home
    if (!home) {
      this.#bot.whisper(username, 'No home set. Stand me on my bed and whisper "sethome".')
      return
    }
    const pos = this.#bot.entity?.position
    const distance = pos ? Math.hypot(pos.x - home.x, pos.z - home.z) : null
    const suffix = distance === null ? '' : ` — I'm ${distance.toFixed(1)} blocks from it`
    this.#bot.whisper(username, `Home is x=${home.x} z=${home.z}${suffix}`)
  }

  #clearHome (username) {
    this.config.kit.home = null
    this.state?.delete('kitHome')
    this.#bot.whisper(username, 'Home cleared; falling back to config.json on next restart.')
    this.log.info(`Owner ${username} cleared the saved home`)
  }
...
class Supervisor {
  ...
  constructor ({ config, notifier, logger, state = null }) {
    this.config = config
    this.notifier = notifier
    this.log = logger
    this.state = state
